(function () {
  ...
  var chapters = [
    {
      ...
      sections: [
        {
          title: '不用学操作，说人话就行',
          paragraphs: ['过去四十年我们用软件，都是软件先摆出一堆固定的按钮和菜单，**你得先学会它的规则**。想做个数据透视表？先学在哪个菜单里、点几下、参数怎么填。学不会，这个功能对你就等于不存在。', '现在反过来了——**你说人话，它去办**。不用记菜单在哪，也不用学函数怎么写。你怎么跟同事交代，就怎么跟它说。', '**Word、Excel、SAP 要你去适应它们，AI 反过来适应你。**这一下换过来，有两件事跟着变了。'],
          bullets: [
            { term: '「不会用」的意思变了', explain: '以前是没学过操作，现在是**没说清楚要什么**。说清楚要求这件事，你带团队、派活的时候天天在做，不用另学。' },
            { term: '但它会出错，而且看不出来', explain: 'Excel 点错了会报错，AI 不会。它每次都是现造的，**造出来很顺，整段也可能是错的**。' },
          ],
          demo: {
            type: 'shift',
            title: '同一件事，两种做法',
        ...
        },
        {
          ...
          scenes: [
            ...
            { icon: 'audience', title: '同一份材料，换个人讲', input: '总部发下来的 30 页季度报告，你只需要讲清其中和我们区相关的三页', assist: '按不同对象重讲一遍重点', confirm: '数字有没有讲错、对象和内容配不配。', example: '可以和 AI 说：请分别用给管理者和给新同事看的方式，讲清这份材料的三个重点。' },
        ...
        },
        {
          ...
          bullets: [
            { term: '那几个概率是从哪来的', explain: 'AI 训练的时候读过海量文字，哪个词后面常跟哪个词，统计过无数遍。所以「上季度的销售」后面接「增长」的概率高，接「下滑」的低。**这是语言习惯的统计，跟我们公司实际卖了多少没有关系。**你那份报表它没读过，公司内部的数据从来没进过它的训练材料。' },
          ...
          demo: {
            type: 'typewriter',
            title: '看它怎么编出一个数字',
            hint: '下面的百分比来自它训练时读过的文字统计，不是我们的销售数据。点一点，看它每一步怎么挑。',
            ...
            verdictSmooth: '每一步都挑了概率最高的，读起来很顺，很像一份真的季度回顾。可是「12%」和「近四成」这两个数，是它一步步挑出来的。我们那份报表里，可能根本没有这两个数字。\n\n那把报表贴给它呢？数字就会跟着报表走，因为正确答案就摆在眼前，被挑中的概率最高。**但它还是在挑，不是在读懂。**报表里没写的部分，它照样会给你编一个。',
            verdictOffTrack: '你挑了概率低的，整句话就歪到别处去了。它一次都没拦你，因为它手上只有「哪个词更像样」这一个标准，没有对错这个概念。',
          },
        },
        {
          title: '关掉对话，它就把你忘了',
          paragraphs: ['它能看到的，只有这一轮对话里的内容。**像同事帮你干活时手边摊开的那摞资料，资料一收走，他什么都不记得了。**（这摞资料有个术语叫「上下文」，你会经常听到。）', '而且这张桌子就那么大。你不停往上放新资料，最早那几张会被挤下去，从桌上掉到地上。它再也看不见了。你这边的感受就是**聊到后面它开始前后矛盾，把你刚否掉的方案又提一遍**。那怎么办？下面三条照着做。'],
          bullets: [
            { term: '常用背景存成一段，每次开新对话先贴', explain: '新开一个对话等于换了个从没见过你的人。把项目是什么、给谁看、有什么忌讳写成一段话存着，比每次重讲快得多。' },
            { term: '最关键的要求放开头，或者结尾再说一遍', explain: '桌子**中间**那部分它最不上心，开头和结尾记得最牢。别把最要紧的那条埋在一大段材料里。' },
            { term: '它开始跑题就换一轮，别硬掰', explain: '你越解释桌子越挤。把已经谈定的结论整理成几句话，新开对话贴进去，相当于给新同事一份交接文档。' },
          ],
          demo2: {
            type: 'context-scale',
            kind: 'note',
            ...
            models: [
              { names: ['Claude Haiku 4.5'], tokens: 200000, label: '20 万 Token', note: '轻量快速档。大概装得下一本 15 万字的书。' },
              { names: ['豆包 seed-2.1-pro', 'Kimi K2.7'], tokens: 256000, label: '25.6 万 Token', note: '和上面一档接近。日常对话、单篇长文都够用。' },
              { names: ['Claude Opus 5', 'GPT-5.6', 'Gemini 3.7', 'Qwen3.8-Max', 'DeepSeek-V4', 'Kimi K3'], tokens: 1000000, label: '100 万 Token', note: '目前主流旗舰基本都在这一档，大概能装五六本长篇小说。' },
            ],
    ...
    },
    {
      ...
      sections: [
        {
          title: '把它当入职第一天的新同事',
          paragraphs: ['一个名校毕业、很聪明、学东西极快的新人——但今天是他第一天上班。他不认识你的老板，不知道我们的口径，没见过我们的材料长什么样。', '而且每次新开对话，都是他的入职第一天。'],
          bullets: [
            { term: '你得先把背景给够', explain: '你不会对新人说「写个材料」就走开。你会告诉他给谁看、什么场合、有什么忌讳。对它也一样。' },
            { term: '「写专业一点」是句废话', explain: '新同事不知道我们这儿「专业」长什么样。是要严谨克制，还是要有点热度？是能用行业黑话，还是要说人话？你不说，他只能猜。' },
  ...
  function renderTypewriterDemo(ownerDocument, root, demo) {
    ...
    function renderStep() {
      ...
      if (stepIndex >= demo.steps.length) {
        ...
        clearNode(verdict);
        var verdictParas = String(verdictText).split('\n\n');
        for (var vIndex = 0; vIndex < verdictParas.length; vIndex += 1) {
          var vPara = element(ownerDocument, 'p', vIndex ? 'lesson-type-verdict-more' : '');
          appendEmphasisText(ownerDocument, vPara, verdictParas[vIndex]);
          verdict.appendChild(vPara);
        }
  ...
  function renderContextScaleDemo(ownerDocument, root, demo) {
    ...
    for (var index = 0; index < demo.models.length; index += 1) {
      ...
      var modelNames = model.names || [model.name];
      for (var nameIndex = 0; nameIndex < modelNames.length; nameIndex += 1) {
        nameNode.appendChild(element(ownerDocument, 'span', 'lesson-scale-chip', modelNames[nameIndex]));
      }
