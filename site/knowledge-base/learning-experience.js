  ...
  var chapters = [
    {
      ...
      sections: [
        {
          title: '不用学操作，说人话就行',
          paragraphs: ['过去四十年，我们用软件是这样的：软件有一堆固定的按钮和菜单，**你得先学会它的规则**。想做个数据透视表？先学在哪个菜单里、点几下、参数怎么填。学不会，这个功能对你就等于不存在。', '现在第一次反过来了：**你说人话，它去办**。不用记菜单在哪，也不用学函数怎么写——你怎么跟同事交代，就怎么跟它说。', '这就是它和 Word、Excel、SAP 最根本的区别：**那些软件要你去适应它，而 AI 反过来适应你**。'],
          bullets: [
            { term: '「不会用」这件事，意思变了', explain: '以前不会用是因为没学过操作。现在不会用，多半是**没说清楚要什么**。这是好消息：说清楚要求是你本来就会的能力，不是新技能。' },
            { term: '它没有功能列表', explain: 'Excel 有明确的功能边界，AI 没有。你能想到怎么描述的活，都可以先丢给它试一次。试错成本就是几十秒。' },
            { term: '但它会出错，而且看不出来', explain: '固定按钮不会给你错的结果，最多是你点错了。而 AI 每次都是现造的，**造得顺不代表造得对**。这是它跟传统软件最大的一个差别，后面几节都在讲这件事。' },
          ],
          demo: {
            type: 'shift',
            title: '同一件事，两种做法',
            ...
            punchline: '省掉的不是打字时间，是**「你得先知道怎么做」**这件事。代价是：结果得你自己把关。',
          },
        },
        {
          title: '四件今天就能上手的活',
          ...
          scenes: [
            ...
            { icon: 'draft', title: '一个想法写成稿', input: '你心里已经想清楚要通知什么，但一直没落成字：要说什么、给谁、什么语气', assist: '写成邮件、汇报或沟通稿初稿', confirm: '事实、语气，还有对外能不能这么说。', example: '可以和 AI 说：请把这个主题写成一封简洁邮件，先给结论，再列三项行动。' },
        ...
        {
          title: '它每写一个词，都是在猜',
          paragraphs: ['你打完一句话，它开始回答。这中间它到底在干什么？', '它把你的话切成一个个碎片，然后根据前面所有内容，**猜下一个碎片最可能是什么**，猜完接上，再猜下一个。整篇回答就是这么一个词一个词长出来的。'],
          bullets: [
            { term: '你说什么，它都能接上', explain: '像相声演员接梗——你抛什么他都接得住，而且接得又快又溜。它的本事是**让这段话顺下去**，不是**保证这话是真的**。所以「答得特别顺」这件事，一点都不说明它答得对。' },
            { term: '它会一本正经地胡说', explain: '它的任务是让这句话**看起来像个答案**，不是确认这件事是真的。没资料的时候它照样能顺畅地写下去，编出一个不存在的数字或出处。而且编的时候语气和说真话时**一模一样**，你看不出来。' },
            { term: '让它算账，它会算错', explain: '问它 7×8，它答得对——这道题它见过几百万遍，等于在背答案。但问它 4783×926，它没背过，就给你一个「看着像那么回事」的数字。**它从头到尾没做过一次计算**，只是在猜这串数字后面该跟什么。所以报表里的加总、百分比，别让它心算：让它写公式，或者你自己按计算器。' },
          ],
          demo: {
            type: 'typewriter',
            title: '看它怎么编出一个数字',
            hint: '它没有你的销售数据。点一点，看它每一步在几个候选里挑一个接上去。',
            ...
            verdictSmooth: '每一步都挑了概率最高的，读起来很顺、很像一份真的季度回顾。但这句话里的「12%」和「近四成」，是它一步步挑出来的，不是从你的报表里查出来的——你手上可能根本没有这两个数。',
            verdictOffTrack: '你挑了概率低的，整句话就歪到别处去了。它一次都没拦你，因为它本来就只是在挑「下一个像样的词」，没有对错的概念。',
          },
        },
        {
          title: '关掉对话，它就把你忘了',
          paragraphs: ['它能看到的，只有这一轮对话里的内容。**像同事帮你干活时手边摊开的那摞资料，资料一收走，他什么都不记得了。**（这摞资料有个术语叫「上下文」，你会经常听到。）', '而且这张桌子就那么大。你不停往上放新资料，最早那几张就会被挤下去——不是被删了，是它再也看不见了。你的感受就是：**聊到后面它开始前后矛盾，把你刚否掉的方案又提一遍。**'],
          bullets: [
          ...
          demo2: {
            ...
            footnote: '还有专门做长文本的档位，比如通义 Qwen-Long 到 1000 万 Token——但那是特例，不是通用旗舰的能力。\n\n**装得下不等于记得清。**桌子再大，中间那部分它照样最容易忽略，关键要求还是要放开头或结尾。\n\n以上为 2026 年 8 月各家官方文档口径，**这些数字变得很快，用之前自己确认一下**。',
          },
          demo: {
            type: 'context-window',
            title: '看它把哪句话丢了',
            ...
            consequence: '看最后一条：它又提滑雪装备了。因为「别提滑雪装备」那句已经被挤下桌，它是真的看不见了——不是不听话。',
          },
        },
      ],
      takeaway: {
        title: '这一章带走什么',
        template: 'AI 到底是什么\n\n它不是一个能查东西的软件。它做的事是：根据你说的话，一个词一个词往下猜——猜得又快又像样，但从头到尾没查过任何资料。\n\n记住这三条就够了：\n\n一、它会编，而且你看不出来\n它编的时候，语气和说真话一模一样，你没法从「听起来怎么样」判断真假。所以数字、日期、人名、出处，见一个核一个，别管它说得多肯定。\n\n二、关掉对话它就忘了\n它只记得这一轮聊过的内容。常用的背景（项目是什么、给谁看、有什么忌讳）存成一段话，每次开新对话先贴一遍，比每次重讲快得多。\n\n三、它只管生成，不管对错\n东西是你发出去的，署名是你的，出了事也是你的。所以别的都能省，最后核对那一步不能省。',
      },
    },
    {
      id: 'ai-boundaries',
