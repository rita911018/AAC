...
assert.equal(
  ...
  4,
  'resources main must contain exactly the Hero, internal Xiao A, featured course, and external resources sections',
);
assert.ok(
...
assert.ok(hasClass(resourcesMainDirectElements[2], 'featured-course'), 'resources main third section must be the featured course');
assert.ok(hasClass(resourcesMainDirectElements[3], 'external-resources'), 'resources main fourth section must be external resources');
assert.equal(
  resourcesPageElements.filter(({ tagName }) => tagName === 'section').length,
  4,
  'resources must contain exactly four sections total',
);
for (const [element, label] of [
  [resourcesHero, 'resources Hero'],
  [resourcesXiaoA, 'resources internal Xiao A'],
  [resourcesPageElements.find((element) => element.tagName === 'section' && hasClass(element, 'featured-course')), 'resources featured course'],
...
assert.ok(
  ...
  'resources order must place Hero and internal Xiao A before external resources',
);
assert.ok(
...
assert.deepEqual(
  xiaoAItems,
  approvedXiaoACapabilities,
  'resources Xiao A capabilities must contain exactly the three approved normalized items',
);
